...
# different window sizes. NUM_AXIL_M is derived from the list length.
# Override AXIL_ADDR_MAP and NUM_AXIL_M before sourcing this script to customise.
if {![info exists NUM_AXIL_M]} { set NUM_AXIL_M 2 }

if {![info exists AXIL_ADDR_MAP]} {
    set AXIL_ADDR_MAP {
        {0x40000000 0x02000000}
        {0x42000000 0x02000000}
        {0x44000000 0x02000000}
    }
}
...
# Create NUM_AXIL_M AXI-Lite master interface ports
set axil_busif_list ""
for {set i 0} {$i < $NUM_AXIL_M} {incr i} {
    set port_name [format "M%02d_AXI_0" $i]
    set port_obj [ create_bd_intf_port -mode Master -vlnv xilinx.com:interface:aximm_rtl:1.0 $port_name ]
    ...
    if {$i == 0} {
        set axil_busif_list $port_name
    } else {
        append axil_busif_list ":$port_name"
    }
}
...
for {set i 0} {$i < $NUM_AXIL_M} {incr i} {
    set port_name [format "M%02d_AXI_0" $i]
    set pin_name  [format "M%02d_AXI"   $i]
    connect_bd_intf_net -intf_net "smartconnect_0_${pin_name}" \
        [get_bd_intf_ports $port_name] [get_bd_intf_pins smartconnect_0/$pin_name]
}
